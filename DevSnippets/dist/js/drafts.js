...
const Drafts = (() => {
    ...
    async function commit(id) {
        if (!id || !_drafts[id]) return false; const d = _drafts[id]; try { if (typeof Storage !== 'undefined' && Storage.editSnippetById) { const payload = {}; ['title','description','code','contentType','blocks','coverImage','blockTitle','fav'].forEach(k => { if (d[k] !== undefined) payload[k] = d[k]; }); Storage.saveStateForUndo(); console.debug('[Drafts] commit payload for', id, payload); const ok = Storage.editSnippetById(id, payload); try { const f = Storage.findSnippetById ? Storage.findSnippetById(id) : null; console.debug('[Drafts] snippet after editSnippetById:', f ? f.item : null); } catch(e) {} if (!ok) {} Storage.save(true); } } catch (e) { console.warn('[Drafts] commit failed', e); } delete _drafts[id]; _saveToStorage(); return true; }
